...
- Requests are accepted only through the `findercreatefile://` URL scheme. The direct-create operation accepts only the four fixed default types. The additional-type operation accepts a fixed catalog ID; both the URL parser and the app resolve it against the same compiled allowlist before use.
